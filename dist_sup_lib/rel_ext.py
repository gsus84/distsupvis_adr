# Taken from: https://github.com/cgpotts/cs224u
...
class Dataset(object):
    ...
    def build_dataset(self, include_positive=True, sampling_rate=0.1, seed=100):
        ...
        for index, rel in enumerate(self.kb.all_relations):
            ...
            print(
                f"relation: {rel} \n"
                f"number positive examples: "
                f"{len(self.kb.get_triples_for_relation(rel))}")
            print(
                "relation examples:\n",
                self.kb.get_triples_for_relation(rel) if len(
                    self.kb.get_triples_for_relation(rel)) <= 10 else self.kb.get_triples_for_relation(rel)[:10]
            )
            print("-----------------------")
    ...
    def count_examples(self):
        ...
        for rel in self.kb.all_relations:
            for kbt in self.kb.get_triples_for_relation(rel):
                ...
                counter[rel] += len(
                    self.corpus.get_examples_for_entities(kbt.sbj, kbt.obj))
                counter[rel] += len(
                    self.corpus.get_examples_for_entities(kbt.obj, kbt.sbj))
        # report results
...
def evaluate(splits, classifier, test_split='dev', sampling_rate=0.1, verbose=True, beta=1):
    ...
    for rel in splits['all'].kb.all_relations:
        ...
        stats = precision_recall_fscore_support(true_labels_by_rel[rel], pred_labels, beta=beta)
        if stats[0].any():
...
def evaluate_predictions(
    ...
    for rel, preds in predictions.items():
        stats = precision_recall_fscore_support(
            test_y[rel], preds, beta=beta
        )  # changed zero_division from 'warn' to 0
        stats = [stat[1] for stat in stats]  # stats[1] is the stat for label True
